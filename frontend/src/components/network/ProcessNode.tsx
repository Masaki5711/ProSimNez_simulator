...
const ProcessNode: React.FC<NodeProps<ProcessNodeData>> = ({ data, selected, id }) => {
  ...
  const handleDoubleClick = (event: React.MouseEvent) => {
    console.log('ProcessNode: Double click detected on node:', data.label);
    // イベントの伝播を停止しないで、ReactFlowのonNodeDoubleClickが処理されるようにする
    // event.stopPropagation(); // この行を削除
  };

  const handleSettingsClick = (event: React.MouseEvent) => {
    event.stopPropagation();
    console.log('ProcessNode: Settings button clicked for node:', id, data);
    // カスタムイベントを発火して、親コンポーネントで設定ダイアログを開く
    const customEvent = new CustomEvent('processNodeSettings', {
      detail: { nodeId: id, nodeData: data }
    });
    console.log('ProcessNode: Dispatching custom event:', customEvent);
